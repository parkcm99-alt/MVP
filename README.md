...
| Architect / Developer / Reviewer / QA API | ✅ 서버 route + 비용 안전 mock fallback + trace telemetry |
...
Planner와 Architect / Developer / Reviewer / QA는 각각 서버 전용 API route를 갖습니다. 비용 보호를 위해 기본값은 모두 mock이며, `ENABLE_LIVE_LLM=true`와 서버 API key를 함께 설정한 경우에만 Claude를 호출합니다. AgentOps, OpenAI, LangGraph, CrewAI 실제 연동은 아직 하지 않았습니다.
...
### 사이드 패널
- **Task Queue** — 태스크 상태(backlog/in-progress/review/done)·담당자 표시 및 역할별 Ask/Review 버튼
...
- **Debug Panel** — Supabase 상태, 마지막 호출 agent/provider, traceRecorded, model, latency/token 표시 (접기/펼치기, mock/trace 실패 경고 표시)
...
| trace_type | 기록 시점 |
|------------|-----------|
| `llm_call` | Planner/Architect/Developer/Reviewer/QA 서버 route에서 Claude live 호출에 성공했을 때 token/latency/model 기록 (`SUPABASE_SERVICE_ROLE_KEY` 우선 사용) |
...
### 역할별 Claude API 준비 상태

Task Queue의 **Review Architecture**, **Ask Developer**, **Ask Reviewer**, **Ask QA** 버튼은 각 서버 전용 route를 호출합니다. 모든 route는 동일한 안전장치를 공유합니다.

- `ENABLE_LIVE_LLM=false` 또는 API key 미설정: 비용 없는 mock 응답
- live 모드: raw JSON 강제, 공통 JSON parser, 8초 timeout, 작은 `max_tokens`
- API key와 raw provider error는 브라우저 응답/로그에 노출하지 않음
- 성공한 live 호출은 `agent_traces`에 역할별 `llm_call` 기록
- Debug Panel은 마지막 호출 role/provider/model/token/latency/trace 상태를 표시

Developer/Reviewer/QA를 포함한 외부 AgentOps/OpenAI/LangGraph/CrewAI 연동은 아직 활성화하지 않았습니다. 기본값은 계속 mock이며, live Claude 호출은 명시적으로 환경변수를 켠 경우에만 발생합니다.
...
`NEXT_PUBLIC_SUPABASE_ANON_KEY`에는 Supabase Dashboard → **Project Settings → API Keys**의 legacy `anon` JWT 또는 새 `publishable` key(`sb_publishable_...`)를 넣습니다. `sk-ant-...` 같은 Claude/Anthropic key나 `service_role`/`sb_secret_...` key를 넣으면 브라우저 번들에 노출되고 Supabase client가 `Invalid API key`로 실패합니다. 실수로 노출했다면 해당 provider key를 즉시 rotate하세요.
...
Debug Panel의 **Trace Correlation Debugger**는 최근 `agent_traces` 100개를 불러와 `session_id`를 선택 가능한 타임라인으로 보여주고, 각 trace의 `agent_id`, `trace_type`, `metadata.task_title`을 함께 비교합니다. 선택한 세션과 매칭되는 Task Queue 항목은 강조되며 관련 Event Log 수와 현재 agent 상태도 표시됩니다. Supabase가 없을 때는 현재 로컬 Event Log를 읽기 전용 local trace로 변환해 UI를 유지합니다.

자동 anomaly 규칙은 다음과 같습니다.

- 최근 호출의 `traceRecorded=false`
- Planner `handoff` 이후 대상 agent의 `decision` 누락
- **Ask Agent** 호출 후 대응하는 `llm_call` 누락
- `latency_ms >= 10000`
- `finalStatus`/`approvalStatus`가 `failed`, `changes_requested`, `needs_more_*` 계열

각 anomaly는 안전한 한 줄 설명과 해결 힌트를 제공합니다. **Create Debug Finding**은 reviewer/QA용 local-only task와 로컬 Event Log 한 줄만 만들며 Supabase에는 쓰지 않습니다. `session_id + anomaly signature`를 브라우저 저장소에 기록해 중복 생성을 막습니다.

**EXPORT**는 선택 세션을 schema version 1의 Sanitized JSON Bundle로 저장하고, **IMPORT**는 read-only analysis mode로만 엽니다. key 이름이나 값이 API key, bearer token, service-role secret, JWT처럼 보이면 재귀적으로 redaction합니다. 손상된 JSON과 지원하지 않는 schema version은 데이터베이스 쓰기 없이 거부됩니다. 그래도 bundle을 외부 공유하기 전에는 내용을 다시 검토하세요.

...
Planner와 Architect는 서버 전용 API route를 통해 Claude live 호출까지 연결되었고, Developer / Reviewer / QA는 아직 mock workflow를 유지합니다. AgentOps, OpenAI, LangGraph, CrewAI 실제 연동은 아직 하지 않았습니다.
...
| trace_type | 기록 시점 |
|------------|-----------|
| `llm_call` | Planner/Architect가 서버 route에서 Claude live 호출에 성공했을 때 token/latency/model 기록 (`SUPABASE_SERVICE_ROLE_KEY` 우선 사용) |

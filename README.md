...
- **Work Request 입력창** — ActionBar 상단에 위치한 업무 요청 textarea
  ...
  - 템플릿 버튼: `미팅 정리` / `영업 제안서` / `운영 리스크 검토` / `자동화 설계` / `거래처 회신 초안` / `정산 검토`
...
`Copy Report`는 위 내용을 Markdown 형식으로 클립보드에 복사합니다. `Download Markdown`은 `ai-agent-report-YYYYMMDD-HHmm.md` 파일로 저장하고, `Print`는 `window.print()` 기반으로 브라우저 인쇄를 실행합니다. Supabase DB 구조는 변경하지 않습니다.

### Work Request 템플릿

Work Request 입력창 오른쪽에는 빠른 템플릿 버튼이 있습니다.

| 템플릿 | 용도 |
|--------|------|
| 미팅 정리 | 결정사항, 담당자, 후속 액션, 리스크 정리 |
| 영업 제안서 | 고객 제안 구조, 가치, 예상 질문 정리 |
| 운영 리스크 검토 | 운영 장애 가능성, 고객 영향, 완화 방안 정리 |
| 자동화 설계 | 시스템 구조, 데이터 흐름, 구현 단계, 테스트 계획 정리 |
| 거래처 회신 초안 | 외부 문의 답변 초안과 확인 항목 정리 |
...
Report 탭 하단에는 최근 5개 보고서가 표시됩니다. 첫 버전은 비용과 DB 변경을 피하기 위해 브라우저 `localStorage`에만 임시 저장합니다.

- 같은 브라우저에서만 유지됩니다.
- 캐시/사이트 데이터 삭제 시 사라질 수 있습니다.
- 항목을 클릭하면 이전 보고서를 다시 볼 수 있습니다.
- 영구 저장이 필요하면 Supabase `agent_reports` 테이블을 별도 migration으로 추가하는 것을 권장합니다.

향후 Supabase 영구 저장 옵션 예시:
...
create policy "Allow service role manage reports"
...
**Debug** 탭 안의 접이식 **AGENT TRACE VIEWER · RECENT 30**은 Supabase 또는 local/mock fallback의 최신 trace를 최대 30건만 표시합니다. 각 카드에서 `trace_type`, `agent_id`, `model`, `latency_ms`, `created_at`, `input_tokens`, `output_tokens`와 민감값을 마스킹한 metadata 요약을 바로 확인할 수 있습니다. 고급 session correlation, Lens warnings, 안전한 bundle import/export 기능은 같은 접이식 영역 안에 유지됩니다.

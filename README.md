...
| Architect/Developer/Reviewer/QA API | ✅ 서버 route + 비용 없는 mock fallback + 선택적 Claude live gate |
...
5개 역할 모두 서버 전용 API route와 mock fallback을 갖습니다. `ENABLE_LIVE_LLM=false`가 기본값이므로 비용은 발생하지 않으며, Claude live 호출은 명시적으로 활성화한 환경에서만 수행됩니다. AgentOps, OpenAI, LangGraph, CrewAI 실제 연동은 아직 하지 않았습니다.
...
- **Ask Architect / Developer / Reviewer / QA** — 역할별 담당 태스크를 구조화된 Claude/mock 응답으로 검토
...
- **Debug Panel** — Supabase 상태, 마지막 호출 역할/provider, traceRecorded, model, latency/token 표시 (접기/펼치기, mock/trace 실패 경고 표시)
- **Trace Correlation Debugger** — 최근 100개 trace를 session별 타임라인으로 묶고 task/event/agent 상태, anomaly, sanitized bundle을 함께 분석
...
## Claude API 준비 및 역할별 연결

> Planner, Architect, Developer, Reviewer, QA는 각자 서버 전용 route를 사용합니다. 기본값은 비용 방지를 위해 mock fallback입니다.

| 항목 | 상태 |
|------|------|
| `@anthropic-ai/sdk` dependency | ✅ 추가 |
| Planner API route | ✅ `POST /api/agents/planner` |
| Specialist API routes | ✅ `/api/agents/architect`, `/developer`, `/reviewer`, `/qa` |
| 공통 JSON parser | ✅ `src/lib/llm/json.ts` (fence 제거 + 안전 파싱) |
...
| trace_type | 기록 시점 |
|------------|-----------|
| `llm_call` | 역할별 서버 route에서 Claude live 호출에 성공했을 때 token/latency/model 기록 (`SUPABASE_SERVICE_ROLE_KEY` 우선 사용) |
...
│   │   ├── json.ts           # 공통 raw JSON parser
│   │   └── agentRoute.ts     # Specialist route 공통 실행기
│   ├── api/
...
| 배포 | Vercel (프로덕션) |
| LLM | Anthropic SDK (`@anthropic-ai/sdk`) — 5개 역할 route, 기본 mock/선택적 live |
...
The Debug Panel now groups the latest 100 `agent_traces` by `session_id` and correlates each timeline with local Task Queue items, Event Log fragments, and agent status. Matching `metadata.task_title` highlights the task. It detects missing trace recording, handoff-without-decision, Ask Agent-without-LLM-call, latency >= 10s, and failed approval/final statuses. **Create Debug Finding** makes one deduplicated local-only reviewer/QA task per session/anomaly signature.

Selected sessions can be exported as a schema-versioned Sanitized JSON Bundle. Keys/values resembling API keys, bearer tokens, credentials, secrets, or service-role tokens are redacted. Imported bundles are read-only analysis data and never write to Supabase tasks, events, or traces; malformed JSON and unsupported schema versions are rejected safely. With no Supabase configuration, local trace telemetry keeps the debugger usable.

...
- 코드에는 Supabase `events`/`agents`/`tasks` 저장과 세 테이블의 멀티 브라우저 Realtime 동기화가 구성되어 있습니다. 기존 문서상 Vercel Production/Supabase LIVE가 기준 환경이지만, 이 변경을 검증한 로컬 환경에는 배포 URL, Vercel CLI/프로젝트 링크, Supabase 환경 변수가 없으므로 외부 Production 상태와 Vercel 로그는 재확인하지 못했습니다.
...
실패한 trace 저장은 5초 timeout 후에도 안전한 `console.warn`과 `false`만 반환하며 앱을 중단하지 않습니다. 향후 AgentOps로 확장할 수 있지만 현재 연결하지 않습니다. 접이식 **Debug Panel**은 Supabase 상태, 마지막 역할/provider, trace 기록 여부, model, latency, token, 호출 시각을 보여줍니다. mock과 `Trace not recorded`는 경고로 표시하며 secret은 표시하지 않습니다.
...
Production/live 검증에는 해당 Vercel 프로젝트 링크/권한 및 Supabase/Anthropic 환경 변수가 필요합니다. 이번 변경에서는 기본 mock 모드의 페이지와 다섯 API fallback, malformed JSON 거부를 확인했습니다.

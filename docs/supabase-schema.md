...
-- agent_traces: MVP 브라우저 Viewer read + handoff/decision insert용
...
### `agent_traces` RLS / key 주의사항

- 서버 Route Handler의 Claude `llm_call` insert는 `SUPABASE_SERVICE_ROLE_KEY`를 우선합니다. service role은 RLS를 우회하므로 **서버 환경변수에만** 두고 절대 `NEXT_PUBLIC_*`나 브라우저에 노출하지 않습니다.
- service role이 비어 있으면 서버는 legacy anon key로 fallback하며 `missing_service_role_key`만 안전하게 경고합니다. 그 경우 위 `anon insert` 정책이 필요합니다.
- 브라우저의 Trace Viewer `select`, Planner handoff/decision `insert`는 `NEXT_PUBLIC_SUPABASE_ANON_KEY`와 위 MVP 정책을 사용합니다. 정책/테이블이 없어도 앱은 local trace/mock으로 유지됩니다.
- 위 `using (true)` / `with check (true)` 정책은 인증 없는 공유 데모용으로만 적합합니다. 공개 production에서는 Supabase Auth를 적용하고 `session_id`를 `auth.uid()` 등 사용자 소유권에 연결한 최소 권한 정책으로 제한하세요. anon update/delete는 열지 않습니다.
- metadata에는 API key, bearer token, service role key, password, credential, raw provider error를 저장하지 않습니다. trace insert 실패는 HTTP status와 redacted body만 경고합니다.

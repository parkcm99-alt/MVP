...
-- agent_traces: browser handoff/decision insert + read-only viewer용 MVP 정책
-- API key/secret은 저장하지 않고, update/delete는 열지 않습니다.
alter table public.agent_traces enable row level security;
create policy "anon read"   on public.agent_traces for select to anon, authenticated using (true);
create policy "anon insert" on public.agent_traces for insert to anon, authenticated with check (true);
```

### `agent_traces` RLS / key 동작

- 서버 Route Handler의 Claude `llm_call` insert는 `SUPABASE_SERVICE_ROLE_KEY`를 우선 사용합니다. service role은 RLS를 bypass하므로 브라우저 번들, `NEXT_PUBLIC_*`, Git에 절대 넣지 않습니다.
- service role key가 없으면 서버는 public anon/publishable key로 fallback하고 `missing_service_role_key`만 경고합니다. 이 fallback과 브라우저의 Planner `handoff`/`decision` insert에는 위 `anon insert` 정책이 필요합니다.
- Trace Correlation Debugger는 브라우저 public key로 최근 trace를 SELECT하므로 `anon read`가 필요합니다. UPDATE/DELETE 정책은 만들지 않습니다.
- 위 `using (true)`/`with check (true)`는 인증 없는 공유 MVP 전용입니다. Production 사용자 격리 단계에서는 Supabase Auth의 `auth.uid()`와 소유 session 매핑에 따라 SELECT/INSERT를 제한하고 public trace 범위를 줄이세요.
- `Invalid API key`가 발생하면 **Project Settings → API Keys**의 `sb_publishable_...` 또는 **Legacy API Keys → anon public** JWT가 `NEXT_PUBLIC_SUPABASE_ANON_KEY`에 들어갔는지, 서버 전용 secret/service_role이 public 변수에 들어가지 않았는지 확인한 뒤 Vercel을 재배포하세요.
...
| 채널명 | 목적 | 이벤트 |
|--------|------|--------|
| `sim-multiplayer` | events/agents/tasks 구독 (중복 방지: session_id 필터) | postgres_changes INSERT/UPDATE |

...
function parsePlannerContent(llm: LlmResponse): { response: PlannerAgentResponse; debugReason?: string } {
  ...
  const parsed = parsePlannerContentObject(llm.content);
  if (!parsed) {
    console.warn('Claude planner response warning: json_parse_failed');
    return {
      response: fallback,
  ...
  const base = safePlannerResponse();
  return {
    response: safePlannerResponse({
      provider: llm.provider,
      summary: normalizeText(parsed.summary, base.summary),
      steps: cleanStringArray(parsed.steps, base.steps, 4),
      risks: cleanStringArray(parsed.risks, base.risks, 3),
      nextAgent: cleanEnum(parsed.nextAgent, ['planner', 'architect', 'developer', 'reviewer', 'qa'] as const, 'architect'),
    }),
  };
}

function parsePlannerContentObject(content: string) {
...
export async function POST(request: Request) {
  ...
  if (llm.provider !== 'claude') {
    return Response.json(withDevDebug(
      withPlannerTelemetry(safePlannerResponse({ provider: 'mock', summary: llm.content }), llm, false, claudeLatencyMs),
      llm.fallbackReason,
      false,
    ));
  }

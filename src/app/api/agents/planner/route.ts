...
import { arrayOfStrings, normalizeSessionId, normalizeText, parseJsonObject } from '@/lib/llm/json';
...
function parsePlannerContent(llm: LlmResponse): { response: PlannerAgentResponse; debugReason?: string } {
  ...
  if (parsed) {
    // A parseable object remains a Claude response even when a field is sparse.
    // Only JSON.parse failure switches to the safe whole-response fallback.
    const defaults = safePlannerResponse();
    const summary = normalizeText(parsed.summary, defaults.summary);
    const steps = arrayOfStrings(parsed.steps, defaults.steps, 4);
    const risks = arrayOfStrings(parsed.risks, [], 3);
    const nextAgent = normalizeText(parsed.nextAgent, defaults.nextAgent);
  ...
  console.warn('Claude planner response failed: json_parse_failed');

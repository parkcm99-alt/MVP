...
import { arrayOfStrings, buildRawJsonPrompt, normalizeText, normalizeUuid, parseJsonObject } from '@/lib/llm/json';
...
function parsePlannerContent(llm: LlmResponse): { response: PlannerAgentResponse; debugReason?: string } {
  ...
  if (!parsed) {
    console.warn('Claude request failed: json_parse_failed (planner)');
  ...
  return {
    response: safePlannerResponse({
      ...
      steps: arrayOfStrings(parsed.steps, fallback.steps, 4),
      risks: Array.isArray(parsed.risks) ? arrayOfStrings(parsed.risks, [], 3) : fallback.risks,
      nextAgent: normalizeText(parsed.nextAgent, fallback.nextAgent, 30).toLowerCase(),
    }),
  };
}
...
export async function POST(request: Request) {
  ...
  const llm = await claudeClient.complete({
    agentRole: ROLE,
    systemPrompt: buildRawJsonPrompt(
  ...
  const traceRecorded = await recordPlannerLlmTrace(llm, taskTitle, claudeLatencyMs, sessionId, traceId);

  if (llm.provider !== 'claude') {
    return Response.json(withDevDebug({
      ...(await buildMockResponse(taskTitle, taskDescription)),
      summary: normalizeText(llm.content, 'Planner가 mock fallback으로 작업을 점검했습니다.'),
      latencyMs: claudeLatencyMs,
    }, llm.fallbackReason, false));
  }

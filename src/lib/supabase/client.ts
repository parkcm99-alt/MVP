...
function getSupabaseConfig(): { url?: string; key?: string; status: SupabaseConfigStatus } {
  ...
  // Supabase accepts legacy anon JWTs and new publishable keys in browser clients.
  // Never accept a secret/service-role/provider key in NEXT_PUBLIC_*.
  const isLegacyAnonKey = key.startsWith('eyJ') && key.split('.').length === 3;

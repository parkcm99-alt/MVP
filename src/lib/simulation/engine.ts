...
/** Planner mini workflows and local findings own their lifecycle. */
function isLoopTask(task: SimTask): boolean {
  return !task.localOnly && task.source !== 'planner-generated'
    && !task.description.includes('[planner-generated]');
}
...
function buildTimeline(): { steps: Step[]; duration: number } {
  ...
  at(0, () => {
    ...
    store().tasks.forEach(t => {
      if (isLoopTask(t) && t.status === 'done') store().updateTask(t.id, { status: 'backlog' });
    });
  });
  ...
  at(18000, () => {
    ...
    store().tasks.forEach(t => {
      if (isLoopTask(t) && t.assignedTo === 'developer' && t.status === 'in_progress') store().updateTask(t.id, { status: 'review' });
    });
  ...
  at(32500, () => {
    ...
    store().tasks.forEach(t => {
      if (isLoopTask(t) && t.assignedTo === 'developer' && t.status === 'review') store().updateTask(t.id, { status: 'done' });
    });
  ...
  at(41000, () => {
    speak('qa', '모든 테스트 통과! 🧪✅');
    store().tasks.forEach(t => {
      if (isLoopTask(t) && t.status === 'in_progress') store().updateTask(t.id, { status: 'done' });
    });
...
export class SimulationEngine {
  ...
  private _runCompletionPhase(onDone?: () => void) {
    ...
    t(0, () => {
      ...
      store().tasks.forEach(task => {
        if (isLoopTask(task) && task.status !== 'done') store().updateTask(task.id, { status: 'done' });
      });
      roles.forEach((id, i) => walkTo(id, MEETING_SEATS[i]));
    });
    t(2000, () => {
  ...
  private seedTasks() {
    if (store().tasks.some(isLoopTask)) return;
    const s = store();

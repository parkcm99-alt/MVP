...
function isSimulationTask(task: { origin?: string; description: string; localOnly?: boolean }): boolean {
  return !task.localOnly && task.origin !== 'planner-generated'
  ...
  private _runCompletionPhase(onDone?: () => void) {
    ...
    t(0, () => {
      ...
      store().tasks.forEach(task => {
        if (!isSimulationTask(task)) return;

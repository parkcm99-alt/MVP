...
const isScriptedTask = (description: string): boolean => !description.includes('[planner-generated]') && !description.includes('[debug-finding]');
...
function buildTimeline(): { steps: Step[]; duration: number } {
  ...
  at(0, () => {
    ...
    store().tasks.forEach(t => {
      if (isScriptedTask(t.description) && t.status === 'done') store().updateTask(t.id, { status: 'backlog' });
    });
  });
  ...
  at(16000, () => {
    ...
    store().tasks.forEach(t => {
      if (!isScriptedTask(t.description)) return;
  ...
  at(18000, () => {
    ...
    store().tasks.forEach(t => {
      if (isScriptedTask(t.description) && t.assignedTo === 'developer' && t.status === 'in_progress') store().updateTask(t.id, { status: 'review' });
    });
  ...
  at(32500, () => {
    ...
    store().tasks.forEach(t => {
      if (isScriptedTask(t.description) && t.assignedTo === 'developer' && t.status === 'review') store().updateTask(t.id, { status: 'done' });
    });
  ...
  at(41000, () => {
    speak('qa', '모든 테스트 통과! 🧪✅');
    store().tasks.forEach(t => {
      if (isScriptedTask(t.description) && t.status === 'in_progress') store().updateTask(t.id, { status: 'done' });
    });
  ...
  private _runCompletionPhase(onDone?: () => void) {
    ...
    t(0, () => {
      ...
      store().tasks.forEach(task => {
        if (isScriptedTask(task.description) && task.status !== 'done') store().updateTask(task.id, { status: 'done' });
      });
      roles.forEach((id, i) => walkTo(id, MEETING_SEATS[i]));
    });
    t(2000, () => {

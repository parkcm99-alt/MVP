...
function isPlannerGenerated(task: { origin?: string; description: string }): boolean {
  return task.origin === 'planner-generated' || task.description.includes('[planner-generated]');
}

/** The scripted demo must not steal a manual/API workflow's agent mid-call. */
function isReserved(id: AgentRole): boolean {
  const task = store().agents[id].currentTask ?? '';
  return task.startsWith('[planner-generated]') || task.startsWith('[ask-agent]') || task.startsWith('Planning:');
}

function scenarioStatus(id: AgentRole, status: AgentStatus) {
  if (!isReserved(id)) store().setStatus(id, status);
}

function speak(id: AgentRole, text: string, ms = 3500) {
  if (isReserved(id)) return after(ms, () => {});
  store().setSpeech(id, text);
  return after(ms, () => {
    if (store().agents[id].speech === text) store().setSpeech(id, null);
  });
}
...
function buildTimeline(): { steps: Step[]; duration: number } {
  ...
  at(8200, () => {
    scenarioStatus('planner', 'meeting');
    scenarioStatus('architect', 'meeting');
  ...
  at(16000, () => {
    scenarioStatus('planner', WORK_STATUS.planner);
    scenarioStatus('architect', WORK_STATUS.architect);
    store().tasks.forEach(t => {
  ...
  at(21000, () => {
    ...
    scenarioStatus('developer', 'meeting');
  });
  at(23500, () => {
    walkTo('developer', DESK_STAND.developer);
  });
  at(25000, () => {
    scenarioStatus('developer', 'coding');
    scenarioStatus('reviewer', 'thinking');
  ...
  at(29500, () => {
    ...
    scenarioStatus('developer', 'blocked');
    if (!isReserved('developer')) store().setSpeech('developer', '버그 확인 중... ⛔');
  ...
  at(36500, () => {
    const roles: AgentRole[] = ['planner', 'architect', 'developer', 'reviewer', 'qa'];
    roles.forEach(id => scenarioStatus(id, 'meeting'));
...
export class SimulationEngine {
  ...
  private _runMeeting(onDone?: () => void) {
    ...
    t(9000, () => {
      roles.forEach(id => scenarioStatus(id, WORK_STATUS[id]));
      if (onDone) onDone();
    });
  }

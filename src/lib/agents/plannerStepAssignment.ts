...
function truncateTitle(value: string, maxLength = 21): string {
...
function summarizeStepForRole(step: string, role: AgentRole): string {
  ...
  // Keep the whole queue title compact while preserving the role at a glance.
  const prefix = `${ROLE_LABELS[role]}: `;
  const title = truncateTitle(withoutLeadingRole || cleaned, Math.max(12, 28 - prefix.length));
  return `${prefix}${title}`;
}

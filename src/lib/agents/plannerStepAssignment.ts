...
function summarizeStepForRole(step: string, role: AgentRole): string {
  ...
  // Include the role in the roughly-20-character budget so long Korean steps
  // stay readable in the narrow queue; the complete step remains in description.
  const title = truncateTitle(withoutLeadingRole || cleaned, Math.max(10, 26 - prefix.length));
  return `${prefix}${title}`;
}

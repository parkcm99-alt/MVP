...
function truncateTitle(value: string, maxLength = 18): string {
...
function summarizeStepForRole(step: string, role: AgentRole): string {
  ...
  // Keep the whole visible title around 20 characters; the complete step stays in description.
  const title = truncateTitle(withoutLeadingRole || cleaned, Math.max(10, 22 - prefix.length));
  return `${prefix}${title}`;
}

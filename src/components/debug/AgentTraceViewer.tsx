...
export default function AgentTraceViewer({refreshKey=null}:Props){
 ...
 const sessions=useMemo(()=>[...new Set(traces.map(t=>t.session_id))], [traces]); const current=useMemo(()=>traces.filter(t=>t.session_id===selected).filter(t=>{const task=tasks.find(x=>x.title===taskTitle(t));return (!lens.role||t.agent_id===lens.role)&&(!lens.traceType||t.trace_type===lens.traceType)&&(!lens.sessionId||t.session_id===lens.sessionId)&&(!lens.status||task?.status===lens.status)&&(!lens.priority||task?.priority===lens.priority)&&textMatch(`${t.agent_id} ${t.trace_type} ${taskTitle(t)} ${JSON.stringify(t.metadata??{})}`,lens.keyword)}),[traces,selected,lens,tasks]); const issues=useMemo(()=>anomalies(current,debug),[current,debug]);
 ...
 return <section className="trace-viewer"><div className="trace-viewer-header"><strong>TRACE CORRELATION DEBUGGER</strong><button className="trace-refresh-btn" onClick={()=>void load()}>REFRESH</button></div><div className="trace-viewer-body">
  <div className="trace-viewer-meta"><select value={selected} onChange={e=>setSelected(e.target.value)}>{sessions.map(s=><option key={s}>{s}</option>)}</select><span>{current.length}/{traces.filter(t=>t.session_id===selected).length} traces {imported?'· READ ONLY':''}</span></div>

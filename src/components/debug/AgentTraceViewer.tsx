...
import { readLocalTraces } from '@/lib/debug/localTraces';
...
function safeWarningLabel(value:string){return SECRET.test(value)||/sk-ant-|bearer\s+|eyJ[A-Za-z0-9_-]+\./i.test(value)?'[redacted task]':value.slice(0,36)}
function sameTask(a:AgentTraceRow,b:AgentTraceRow){const x=taskTitle(a).toLowerCase(),y=taskTitle(b).toLowerCase();return !x||!y||x===y}
function anomalies(traces:AgentTraceRow[], lastCall:{traceRecorded:boolean|null;lastPlanAt:number|null}):Anomaly[]{
 ...
 traces.filter(t=>t.trace_type==='tool_use'&&String(t.metadata?.action??'').includes('ask_agent')).forEach(ask=>{if(!traces.some(t=>t.trace_type==='llm_call'&&t.agent_id===ask.agent_id&&new Date(t.created_at)>=new Date(ask.created_at)&&sameTask(ask,t)))add(`missing-llm:${ask.id}`,`${ask.agent_id} Ask Agent 뒤 llm_call이 보이지 않습니다.`,'ENABLE_LIVE_LLM, API route, trace insert 결과를 확인하세요.')});
...
export default function AgentTraceViewer({refreshKey=null}:Props){
 const [traces,setTraces]=useState<AgentTraceRow[]>([]),[selected,setSelected]=useState(''),[error,setError]=useState(''),[imported,setImported]=useState(false),[collapsed,setCollapsed]=useState(false);
 ...
 const localRows=useCallback(()=>{const session=getSessionId();const rows:AgentTraceRow[]=[...readLocalTraces(),...events.slice(-LIMIT).reverse().map(e=>({id:`local-${e.id}`,session_id:session,agent_id:e.agentId,trace_type:e.type==='planning'?'handoff':e.type==='review'?'decision':'tool_use',input_tokens:null,output_tokens:null,latency_ms:null,model:'local',metadata:{task_title:tasks.find(t=>e.message.includes(t.title))?.title??'',local:true},created_at:new Date(e.timestamp).toISOString()} as AgentTraceRow))];if(debug.lastPlanAt)rows.unshift({id:`local-llm-${debug.lastPlanAt}`,session_id:session,agent_id:debug.agentId??'planner',trace_type:'llm_call',input_tokens:debug.inputTokens,output_tokens:debug.outputTokens,latency_ms:debug.latencyMs,model:debug.model??debug.provider??'mock',metadata:{task_title:tasks.find(t=>t.assignedTo===debug.agentId)?.title??'',traceRecorded:debug.traceRecorded,local:true},created_at:new Date(debug.lastPlanAt).toISOString()});return [...new Map(rows.map(r=>[r.id,r])).values()].sort((a,b)=>Date.parse(b.created_at)-Date.parse(a.created_at)).slice(0,LIMIT)},[debug,events,tasks]);
 ...
 useEffect(()=>{const receive=(event:Event)=>{if(imported)return;const row=(event as CustomEvent<AgentTraceRow>).detail;if(!row)return;setTraces(previous=>[row,...previous.filter(t=>t.id!==row.id)].slice(0,LIMIT));setSelected(value=>value||row.session_id)};window.addEventListener('local-agent-trace',receive);return()=>window.removeEventListener('local-agent-trace',receive)},[imported]);
 ...
 const sessions=useMemo(()=>[...new Set(traces.map(t=>t.session_id))], [traces]); const requestedSession=lens.sessionId.trim(); const effectiveSelected=requestedSession&&sessions.includes(requestedSession)?requestedSession:selected; const selectedTraces=useMemo(()=>traces.filter(t=>t.session_id===effectiveSelected),[traces,effectiveSelected]); const current=useMemo(()=>selectedTraces.filter(t=>traceMatchesLens(t,lens,tasks)),[selectedTraces,lens,tasks]); const issues=useMemo(()=>anomalies(selectedTraces,debug),[selectedTraces,debug]);
 ...
 function finding(){const a=issues.find(issue=>{const sig=`trace-finding:${effectiveSelected}:${issue.signature}`;let saved=false;try{saved=localStorage.getItem(sig)==='1'}catch{}return !saved&&!tasks.some(t=>t.description.includes(sig))});if(!a||imported)return;const sig=`trace-finding:${effectiveSelected}:${a.signature}`;useSimStore.getState().addLocalTask({title:`Trace finding: ${a.text.slice(0,24)}`,description:`[local-only] ${sig} ${a.hint}`,assignedTo:a.role,status:'backlog',priority:'high'});useSimStore.getState().addEvent({agentId:a.role,agentName:agents[a.role].name,agentColor:agents[a.role].primaryColor,type:'review',message:`[Local Debug Finding] ${a.text}`});try{localStorage.setItem(sig,'1')}catch{}}
 function exportBundle(){const b:Bundle={schemaVersion:1,exportedAt:new Date().toISOString(),sessionId:effectiveSelected,traces:selectedTraces};const blob=new Blob([JSON.stringify(redact(b),null,2)],{type:'application/json'});const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`trace-${effectiveSelected.slice(0,8)||'session'}.json`;a.click();URL.revokeObjectURL(a.href)}
 ...
 const lensWarnings=useMemo(()=>{const out:string[]=[];for(const t of tasks.filter(x=>taskMatchesLens(x,{...lens,traceType:'',sessionId:''},traces))){const label=safeWarningLabel(t.title);const linked=traces.filter(x=>taskTitle(x)===t.title);const scoped=linked.filter(x=>(!lens.sessionId||x.session_id===lens.sessionId.trim())&&(!lens.traceType||x.trace_type===lens.traceType));if(!scoped.length)out.push(`Task “${label}” has no related trace for this lens`);else if(t.assignedTo&&scoped.some(x=>x.agent_id!==t.assignedTo))out.push(`Agent role mismatch: “${label}”`);if(!events.some(e=>e.message.includes(t.title)))out.push(`Task “${label}” has no related event`)}if(lens.sessionId&&traces.length&&!traces.some(t=>t.session_id===lens.sessionId.trim()))out.push('sessionId mismatch');return [...new Set(out)].slice(0,6)},[tasks,events,traces,lens]);
 ...
 return <section className={`trace-viewer${collapsed?' trace-viewer--collapsed':''}`}><div className="trace-viewer-header"><strong>AGENT TRACE VIEWER · RECENT 30</strong><div><button className="trace-refresh-btn" onClick={()=>void load()}>REFRESH</button><button className="trace-refresh-btn" onClick={()=>setCollapsed(value=>!value)} aria-expanded={!collapsed}>{collapsed?'OPEN':'CLOSE'}</button></div></div>{!collapsed&&<div className="trace-viewer-body">
  <div className="trace-viewer-meta"><select value={effectiveSelected} onChange={e=>setSelected(e.target.value)}>{sessions.map(s=><option key={s}>{s}</option>)}</select><span>{current.length}/{traces.length} traces {imported?'· READ ONLY':''}</span></div>
  ...
  <div className="trace-list">{current.length===0&&<div className="lens-empty">No matching traces · <button onClick={clearLens}>Clear all</button></div>}{current.map(t=>{const metadata=JSON.stringify(redact(t.metadata??{}));return <article className="trace-card" key={t.id}><div className="trace-card-top"><span className={`trace-badge ${badge(t.trace_type)}`}>{highlightParts(t.trace_type,lens.keyword).map((part,index)=>part.match?<mark key={index}>{part.text}</mark>:part.text)}</span><strong>{highlightParts(t.agent_id,lens.keyword).map((part,index)=>part.match?<mark key={index}>{part.text}</mark>:part.text)}</strong><time>{formatKstTime(t.created_at)} KST</time></div><p>{highlightParts(`${taskTitle(t)||'task —'} · ${t.model??'model —'} · ${t.latency_ms??'—'}ms`,lens.keyword).map((part,index)=>part.match?<mark key={index}>{part.text}</mark>:part.text)}</p><p className="trace-card-metrics">input_tokens: {t.input_tokens??'—'} · output_tokens: {t.output_tokens??'—'}</p><code className="trace-card-metadata">metadata: {metadata.length>180?`${metadata.slice(0,180)}…`:metadata}</code></article>})}</div>
 </div>}</section>
}

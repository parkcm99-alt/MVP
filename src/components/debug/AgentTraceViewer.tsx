...
function validBundle(value:unknown):value is Bundle{
 ...
 return b.traces.every(t=>Boolean(t&&typeof t==='object'&&typeof t.id==='string'&&t.session_id===b.sessionId&&typeof t.agent_id==='string'&&TRACE_TYPES.has(t.trace_type)&&typeof t.created_at==='string'&&Number.isFinite(Date.parse(t.created_at))&&(t.metadata===null||t.metadata===undefined||typeof t.metadata==='object')));
}
...
export default function AgentTraceViewer({refreshKey=null}:Props){
 ...
 const sessions=useMemo(()=>[...new Set(traces.map(t=>t.session_id))], [traces]); const selectedTraces=useMemo(()=>traces.filter(t=>t.session_id===selected),[traces,selected]); const current=useMemo(()=>selectedTraces.filter(t=>{const task=tasks.find(x=>x.title===taskTitle(t));return (!lens.role||t.agent_id===lens.role)&&(!lens.traceType||t.trace_type===lens.traceType)&&(!lens.sessionId||t.session_id===lens.sessionId)&&(!lens.status||task?.status===lens.status)&&(!lens.priority||task?.priority===lens.priority)&&textMatch(`${t.agent_id} ${t.trace_type} ${taskTitle(t)} ${JSON.stringify(t.metadata??{})}`,lens.keyword)}),[selectedTraces,lens,tasks]); const issues=useMemo(()=>anomalies(selectedTraces,debug),[selectedTraces,debug]);
 ...
 const lensWarnings=useMemo(()=>{const out:string[]=[];for(const t of tasks.filter(x=>(!lens.role||x.assignedTo===lens.role)&&(!lens.status||x.status===lens.status)&&(!lens.priority||x.priority===lens.priority)&&textMatch(`${x.title} ${x.description}`,lens.keyword))){const linked=current.filter(x=>taskTitle(x)===t.title);if(!linked.length)out.push(`Task “${t.title.slice(0,36)}” has no related trace`);else if(t.assignedTo&&linked.some(x=>x.agent_id!==t.assignedTo))out.push(`Agent role mismatch: “${t.title.slice(0,36)}”`);if(!events.some(e=>e.message.includes(t.title)))out.push(`Task “${t.title.slice(0,36)}” has no related event`)}if(lens.sessionId&&traces.length&&!traces.some(t=>t.session_id===lens.sessionId))out.push('sessionId mismatch');return [...new Set(out)].slice(0,6)},[tasks,events,current,traces,lens]);

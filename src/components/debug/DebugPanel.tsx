...
export default function DebugPanel() {
  ...
  const lastPlanAt = useDebugStore(s => s.lastPlanAt);
  const { filtered } = useOperationsData();
    ...
    <section className={`debug-panel${collapsed ? ' debug-panel--collapsed' : ''}`}>
      ...
      {!collapsed && (
        <div className="debug-panel-body">
          ...
          <div className="debug-row">
            <span>Last Plan</span>
            <strong>{lastPlanAt ? `${formatKstTime(lastPlanAt)} KST` : '—'}</strong>
          </div>
          <div className="debug-row">
          ...
          {filtered.warnings.length > 0 && (
            <div className="lens-warnings"><strong>Lens warnings · local only</strong>{filtered.warnings.map(warning => <span key={warning}>{warning}</span>)}</div>
          )}

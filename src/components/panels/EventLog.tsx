...
export default function EventLog() {
  ...
  const visible = events.filter(e => (!lens.role || e.agentId === lens.role) && (!keyword || e.message.toLowerCase().includes(keyword)));
    ...
    >
      ...
      <div className="panel-header" style={{ flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          <span>📝 EVENT LOG</span>
          <span className="panel-badge">{visible.length}/{events.length}</span>
        </div>
        <button
        ...
        >
          {visible.length === 0 && (
            <span style={{ color: '#475569', fontFamily: 'monospace', fontSize: 10 }}>
              No matching events. Clear all to reset.
            </span>
          )}
          {visible.map(evt => {

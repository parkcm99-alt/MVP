...
export default function TaskQueue() {
  ...
  const keyword = lens.keyword.trim().toLowerCase();
  const filteredTasks = tasks.filter(t => (!lens.role || t.assignedTo === lens.role) && (!lens.status || t.status === lens.status) && (!lens.priority || t.priority === lens.priority) && (!lens.sessionId || getSessionId().includes(lens.sessionId)) && (!keyword || `${t.title} ${t.description}`.toLowerCase().includes(keyword)));
  ...
  return (
    <div className="panel">
      ...
      <div className="panel-body" style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
        {filteredTasks.length === 0 && <span className="lens-empty">No matching tasks. Clear all to reset.</span>}
            ...
            <div key={status}>
              ...
              {group.map(task => (
                <div
                  ...
                  style={{
                    background: style.bg,
                    border: task.title === highlightedTaskTitle ? '2px solid #FBBF24' : `1px solid ${style.text}33`,
                    boxShadow: task.title === highlightedTaskTitle ? '0 0 8px rgba(251,191,36,.45)' : undefined,
                ...
                >
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 6 }}>
                    <span style={{ fontSize: 10, color: style.text, fontFamily: 'monospace', fontWeight: 'bold', lineHeight: 1.25, overflowWrap: 'anywhere' }}>
                      {keyword && task.title.toLowerCase().includes(keyword) ? <mark>{task.title}</mark> : task.title}
                    </span>

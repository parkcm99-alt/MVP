...
export default function AgentSprite({ shirtColor, pantColor, status, flipX = false }: Props) {
  ...
  const animClass =
    ...
    status === 'coding'    ? 'animate-agent-code'    :
    status === 'reviewing' ? 'animate-agent-think'   :
    status === 'testing'   ? 'animate-agent-think'   :
    status === 'thinking'  ? 'animate-agent-think'   :
    status === 'meeting'   ? 'animate-agent-meet'    :
    status === 'blocked'   ? 'animate-agent-blocked' : '';

  // blocked = red tint on shirt
  const effectiveShirt = status === 'blocked' ? '#DC2626' : shirtColor;
  ...
  return (
    ...
    >
      ...
      {/* ── body (shirt) ── */}
      <rect x="6"  y="16" width="20" height="16" fill={effectiveShirt} />
      {/* collar */}
      <rect x="11" y="16" width="10" height="5"  fill={white} />
      {/* ── left arm ── */}
      <rect x="0"  y="18" width="6"  height="12" fill={effectiveShirt} />
      {/* ── right arm ── */}
      <rect x="26" y="18" width="6"  height="12" fill={effectiveShirt} />
      {/* ── hands ── */}

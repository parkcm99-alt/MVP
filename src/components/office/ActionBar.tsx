...
function pickHighestPriorityTask(tasks: SimTask[]): SimTask | undefined {
  return tasks
    .filter(task => task.status !== 'done' && !task.localOnly)
    .sort((a, b) => {
...
export default function ActionBar() {
  ...
  const clearFilters = useDebugStore(s => s.clearFilters);
  const resetLensContext = useDebugStore(s => s.resetLensContext);
  const imported = useDebugStore(s => s.importedBundle !== null);
  ...
  useEffect(() => {
    if (!imported) return;
    // Import is an isolated, read-only analysis view. Stop pending local work;
    // the operator can explicitly resume after exiting it.
    plannerWorkflowTimers.current.forEach(window.clearTimeout);
    plannerWorkflowTimers.current = [];
  ...
  async function askPlanner() {
    ...
    clearFilters();
    startAgentCall({ id: callId, sessionId, agentId: 'planner', taskTitle, calledAt: Date.now() });
    setPlannerBusy(true);
    ...
    try {
      const response = await fetch('/api/agents/planner', {
        ...
        body: JSON.stringify({ taskTitle, taskDescription, sessionId, session_id: sessionId, traceId: callId }),
      });
      const result = await response.json() as PlannerAgentResponse;
      if (!response.ok || result.role !== 'planner' || (result.provider !== 'mock' && result.provider !== 'claude')) throw new Error('invalid_response');
      const telemetry = {
      ...
      recordAgentResponse(callId, 'planner', telemetry);
      recordClientTrace({
        id: callId, sessionId, agentId: 'planner', traceType: 'llm_call', ...telemetry,
        metadata: { provider: result.provider, task_title: taskTitle, traceRecorded: telemetry.traceRecorded },
        persist: false,
      });
      ...
      recordAgentResponse(callId, 'planner', {
        provider: 'mock',
        traceRecorded: false,
        model: 'mock-fallback',
        latencyMs: null,

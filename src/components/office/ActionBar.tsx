...
import { recordAgentInvocation, recordClientTrace, useTraceStore } from '@/store/traceStore';
...
function schedulePlannerGeneratedWorkflow(
  ...
  const agentOffsets: Partial<Record<AgentRole, number>> = {};
  tasks.forEach((task, index) => {
    const agentId = task.assignedTo ?? 'planner';
    const taskLabel = `${PLANNER_GENERATED_MARKER} ${task.title}`;
    // Queue work per agent so two generated steps cannot overwrite one another.
    const startDelay = (agentOffsets[agentId] ?? 0) * 7000 + index * 120;
    agentOffsets[agentId] = (agentOffsets[agentId] ?? 0) + 1;
...
export default function ActionBar() {
  ...
  const readOnlyImport = useTraceStore(s => Boolean(s.importedBundle));
  ...
  useEffect(() => {
    if (!readOnlyImport) return;
    plannerWorkflowTimers.current.forEach(window.clearTimeout);
    plannerWorkflowTimers.current = [];
    simulationEngine.stop();
  }, [readOnlyImport]);

  function stopGeneratedWorkflow() {
    plannerWorkflowTimers.current.forEach(window.clearTimeout);
    plannerWorkflowTimers.current = [];
  }

  async function askPlanner() {
    if (plannerBusy || readOnlyImport) return;
    ...
    try {
      ...
      recordAgentInvocation({
        sessionId, agentId: 'planner', taskTitle, calledAt,
      ...
      if (createdTasks.length) {
        // The scripted sprint and generated queue both own the same sprites.
        simulationEngine.stop();
        schedulePlannerGeneratedWorkflow(createdTasks, plannerWorkflowTimers, sessionId);
      }
      useTraceStore.getState().selectSession(sessionId);
      ...
      recordAgentInvocation({ sessionId, agentId: 'planner', taskTitle, calledAt, provider: null, traceRecorded: false, failed: true });
      eventBus.emit('agent.planning', {
        agentId: 'planner',
        data: {
          summary: speech,
  ...
  return (
    <div className="action-bar">
      ...
      <div className="action-bar-buttons">
        <ActionBtn
          variant="start"
          onClick={() => { stopGeneratedWorkflow(); simulationEngine.startSprint(); }}
          title="새 스프린트 시작"
          active={isRunning}
          disabled={readOnlyImport}
        >
          ▶ Start Sprint
        </ActionBtn>
        <ActionBtn
          variant="meeting"
          onClick={() => { stopGeneratedWorkflow(); simulationEngine.callMeeting(); }}
          title="전체 미팅 소집"
          disabled={readOnlyImport}
        >
          💬 Call Meeting
        ...
        <ActionBtn
          ...
          disabled={plannerBusy || readOnlyImport}
        >
          {plannerBusy ? 'Planning...' : 'Plan with Claude'}
        </ActionBtn>
        <ActionBtn
          variant="complete"
          onClick={() => { stopGeneratedWorkflow(); simulationEngine.completeSprint(); }}
          title="스프린트 즉시 완료"
          disabled={readOnlyImport}
        >
          ✓ Complete
        ...
        <ActionBtn
          variant="reset"
          onClick={() => {
            stopGeneratedWorkflow();
            generatedPlannerResponses.current.clear();
            useTraceStore.getState().clearLocal();

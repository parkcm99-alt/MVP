...
function createTasksFromPlannerSteps(
  ...
  createdTasks.forEach(task => {
    ...
    void insertAgentTrace({
      ...
      if (!recorded) useTraceDebugStore.getState().addLocalTrace({
        agent_id: 'planner', trace_type: 'handoff', metadata,
      });
    });
  });
  ...
  tasks.forEach((task, index) => {
    ...
    timers.current.push(window.setTimeout(() => {
      ...
      void insertAgentTrace({
        ...
        if (!recorded) useTraceDebugStore.getState().addLocalTrace({
          agent_id: agentId, trace_type: 'decision', metadata,
        });
      });
...
export default function ActionBar() {
  ...
  async function askPlanner() {
    ...
    setPlannerBusy(true);
    addLocalTrace({ agent_id: 'planner', trace_type: 'tool_use', metadata: { action: 'Ask Agent', task_title: taskTitle } });
    ...
    try {
      ...
      addLocalTrace({ agent_id: 'planner', trace_type: 'tool_use', metadata: { action: 'Ask Agent failed', task_title: taskTitle, traceRecorded: false } });
      eventBus.emit('agent.planning', {
        agentId: 'planner',
        data: {
          summary: speech,
  ...
  async function askSpecialist(role: Exclude<AgentRole, 'planner'>) {
    if (agentBusy) return;
    const roleTask = tasks.find(task => task.assignedTo === role && task.status !== 'done')
      ?? pickHighestPriorityTask(tasks);
    const taskTitle = roleTask?.title ?? `${role} 검토`;
    const taskDescription = roleTask?.description ?? '현재 MVP 작업을 검토하고 다음 단계를 제안합니다.';
    const previous = useSimStore.getState().agents[role];
    const previousStatus = previous.status;
    const previousTask = previous.currentTask;
    const workingStatus: AgentStatus = role === 'developer' ? 'coding' : role === 'qa' ? 'testing' : 'reviewing';
    setAgentBusy(role);
    addLocalTrace({ agent_id: role, trace_type: 'tool_use', metadata: { action: 'Ask Agent', task_title: taskTitle } });
    useSimStore.getState().setStatus(role, workingStatus);
    useSimStore.getState().setTask(role, `Claude review: ${taskTitle}`);
    ...
    try {
      ...
      const summary = typeof result.summary === 'string' ? result.summary : `${role} 검토가 완료되었습니다.`;
      ...
      eventBus.emit('agent.message', { agentId: role, data: { message: `${labels[role]}: ${summary}`, provider } });
      const detailFields: Record<string, Array<[string, string]>> = {
        architect: [['architectureNotes', '[Architect] 설계 메모'], ['dataFlow', '[Architect] 데이터 흐름']],
      ...
      for (const [field, label] of detailFields[role]) {
        const value = result[field];
        const display = Array.isArray(value) ? value.join(' · ') : typeof value === 'string' ? value : '';
        if (display) eventBus.emit('agent.message', { agentId: role, data: { message: `${label}: ${display}` } });
      }
      ...
      addLocalTrace({ agent_id: role, trace_type: 'tool_use', metadata: { action: 'Ask Agent failed', task_title: taskTitle, traceRecorded: false } });
      eventBus.emit('agent.message', { agentId: role, data: { message: `[${role}] 호출 실패. mock simulation은 계속됩니다.` } });
    } finally {
      const current = useSimStore.getState().agents[role];
      if (current.currentTask === `Claude review: ${taskTitle}`) {
      ...
      <div className="action-bar-buttons">
        ...
        {(['architect', 'developer', 'reviewer', 'qa'] as const).map(role => (
          <ActionBtn
            key={role}
            variant={role}
            onClick={() => { void askSpecialist(role); }}
            title={`${role} 담당 태스크를 Claude/mock으로 검토`}
            disabled={agentBusy !== null}
          >
            {agentBusy === role ? 'Working...' : `Ask ${role[0].toUpperCase()}${role.slice(1)}`}
          </ActionBtn>
        ))}
        <ActionBtn
          variant="meeting"

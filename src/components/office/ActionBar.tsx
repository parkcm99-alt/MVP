...
function schedulePlannerGeneratedWorkflow(
  ...
  const roleOffsets: Record<AgentRole, number> = {
    planner: 0, architect: 0, developer: 0, reviewer: 0, qa: 0,
  };
  tasks.forEach((task, index) => {
    const agentId = task.assignedTo ?? 'planner';
    const taskLabel = `${PLANNER_GENERATED_MARKER} ${task.title}`;
    // Serialize work per agent while allowing different roles to work concurrently.
    const startDelay = roleOffsets[agentId] * 7000 + index * 120;
    roleOffsets[agentId] += 1;
...
export default function ActionBar() {
  ...
  const failAgentCall = useDebugStore(s => s.failAgentCall);
  ...
  async function askPlanner() {
    ...
    setPlannerBusy(true);
    // Planner-generated mini workflows own agent state until finished.
    // The existing mock sprint remains available through Start Sprint / Resume.
      ...
      <div className="action-bar-buttons">
        ...
        <ActionBtn
          ...
          onClick={() => {
            ...
            useLensStore.getState().clear();
            useDebugStore.getState().setSelectedSessionId(null);

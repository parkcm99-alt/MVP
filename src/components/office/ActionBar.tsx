  ...
  const createdTasks = cleanSteps.flatMap((step, index) =>
    assignPlannerStep(step).map(spec =>
      store.addTask({
        ...
        origin: 'planner-generated',
      }),
    ),
  );

  createdTasks.forEach(task => {
    void recordClientTrace({
...
function schedulePlannerGeneratedWorkflow(
  ...
  tasks.forEach((task, index) => {
    ...
    timers.current.push(window.setTimeout(() => {
      ...
      if (agentId === 'architect') void askSpecialist('architect', task, false);
    }, workDelay));
...
export default function ActionBar() {
  ...
  async function askPlanner() {
    ...
    const invocationId = useDebugStore.getState().beginInvocation({
      sessionId, agentId: 'planner', taskTitle, startedAt: Date.now(),
    });
    try {
      ...
      useDebugStore.getState().completeInvocation(invocationId, result.provider, result.traceRecorded ?? false);
      mirrorLlmTrace({
        sessionId, agentId: 'planner', inputTokens: result.inputTokens, outputTokens: result.outputTokens,
        latencyMs: result.latencyMs, model: result.model,
        metadata: { task_title: taskTitle, provider: result.provider, traceRecorded: result.traceRecorded ?? false, local_only: true },
      });
      ...
      <div className="action-bar-buttons">
        ...
        <ActionBtn
          ...
          onClick={() => {
            ...
            useDebugStore.getState().resetRuntime();
            useOperationsStore.getState().clearFilters();
          }}
          title="오피스 초기화"
        >
          ↺ Reset

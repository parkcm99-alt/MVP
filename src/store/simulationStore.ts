...
export const useSimStore = create<SimulationStore>((set, get) => ({
  ...
  addTask: (t) => {
    const newTask: SimTask = { ...t, sessionId: t.sessionId ?? getSessionId(), id: uuid(), createdAt: Date.now(), updatedAt: Date.now() };
    set(s => ({ tasks: [...s.tasks, newTask] }));
    void upsertTask(newTask);
    return newTask;
  },

  // Debug findings deliberately never enter the persistence/Realtime path.
  addLocalTask: (t) => {
    const newTask: SimTask = { ...t, sessionId: t.sessionId ?? getSessionId(), localOnly: true, id: uuid(), createdAt: Date.now(), updatedAt: Date.now() };
    set(s => ({ tasks: [...s.tasks, newTask] }));
    return newTask;
  },
  ...
  addEvent: (e) =>
    set(s => ({
      events: [
        { ...e, sessionId: e.sessionId ?? getSessionId(), id: uid(), timestamp: Date.now() },
        ...s.events,
      ],
    })),

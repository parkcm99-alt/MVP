...
interface LensStore { filters: LensFilters; setFilter: (key: keyof LensFilters, value: string) => void; clear: () => void }
export const useLensStore = create<LensStore>(set => ({ filters: initial, setFilter: (key, value) => set(s => ({ filters: { ...s.filters, [key]: value } })), clear: () => set({ filters: initial }) }));
